-- Drop the CHECK constraint on events.type so new EventType values can be
-- added without a table rebuild. Validation moves to the application layer
-- (Zod schema + TypeScript enum). Rename legacy 'bypassed' rows to 'dismissed'.
...
SELECT "id", "uuid", "ts",
    CASE WHEN "type" = 'bypassed' THEN 'dismissed' ELSE "type" END,
    "repo_full_name", "pr_number", "correlation_id", "request_id", "version", "payload", "metadata", "pull_request_id"
FROM "events";
