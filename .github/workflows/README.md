...
Triggered by `pull_request_target` `edited` events where `changes.base != null` (base branch was changed) and `pull_request.base.ref == 'main'`. Each comment includes structured metadata (trigger name, timestamp, previous base) in an HTML marker block, letting downstream systems distinguish restacker-triggered reviews from other sources.
