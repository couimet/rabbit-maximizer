...
describe('getConfig', () => {
  ...
  it('returns configured values when non-default', async () => {
    startServer(makeConfig({ PAUSE_NOTIFICATION_INITIAL_DELAY_SEC: 3600, PAUSE_NOTIFICATION_REPEAT_INTERVAL_SEC: 600 }));
    ...
    expect(res.status).toBe(StatusCodes.OK);
    expect(await res.json()).toStrictEqual({
      pauseNotificationInitialDelaySec: 3600,
      pauseNotificationRepeatIntervalSec: 600,
    });
  });
