...
describe('RecentlyTriggered', () => {
  ...
  describe('data', () => {
    ...
    it('shows manual badge when trigger_source is dashboard_retrigger_now', async () => {
    ...
    it('shows em dash when no retrigger_comment_url', async () => {
      const item = makeItem({ retrigger_comment_url: undefined });
      mockTriggeredEndpoint({ data: [item], total: 1, page: 1, pageSize: 50 });
      render(<RecentlyTriggered />);
    ...
    it('shows completed status dot when status is completed', async () => {
      const item = makeItem({ status: QueueStatus.completed });
      mockTriggeredEndpoint({ data: [item], total: 1, page: 1, pageSize: 50 });
      render(<RecentlyTriggered />);

      await waitFor(() => expect(screen.getByTitle('Completed')).toBeInTheDocument());
    });
  });
